...
|---|---|---|
| `fetchMapping` | `submissionSysId`, optional `dataExtractSysId` | Resolves version via `_resolveDataExtraction`. Runs MODEL2AUDIT only when active. Queries line items by `parent = selectedDataExtractSysId`, ordered by `sequence_final` then `field_name_final` (no row limit — extractions can exceed 500; observed 874). Returns `mapping`, `versions[]`, `selectedDataExtract: {sys_id, active}`, `submissionStatusChoice`, `config` (editable status arrays). |
