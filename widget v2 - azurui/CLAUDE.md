...
|---|---|---|
| `fetchMapping` | `submissionSysId`, optional `dataExtractSysId` | Resolves version via `_resolveDataExtraction`. Runs MODEL2AUDIT only when active. Queries line items by `parent = selectedDataExtractSysId`, ordered by `sequence_final` then `field_name_final` (no row limit — extractions can exceed 500; observed 874). **Line items whose `section_name_final` starts with `Locations` are excluded** (strict prefix, filtered in the JS loop — see below). Returns `mapping`, `versions[]`, `selectedDataExtract: {sys_id, active}`, `submissionStatusChoice`, `config` (editable status arrays). |
...
The live rule is centralized in **`c.needsCommentary(field)`** — a pure, non-mutating getter (`qa_override_value` non-empty AND `commentary` empty). It is safe to call from `ng-class` / `title` and is the single definition of the rule reused by all layers below.
...
2. **Complete button disable + live highlight** — `c.hasValidationErrors()` is a pure (non-mutating, digest-cycle-safe) getter wired into `ng-disabled`, delegating the QA/commentary check to `c.needsCommentary`. **Only blocks while fields are in editable mode** — short-circuits to `false` when `c.isAnyFieldEditable()` is false (read-only spans, not inputs), since a stale flag isn't fixable there. When editable, returns true if any field has `validationError` OR any field `needsCommentary`. The tooltip comes from **`c.completeDisabledReason()`** (read-only → "Switch to the active version…"; else missing-commentary → "Add Commentary for each QA Override value before completing"; else server error → "Fix validation errors before completing"). Both the **QA Override input** and the **Commentary input** get a red `.validation-error` border *live* via `c.needsCommentary(field)` (not only after blur), and the commentary placeholder switches to "Commentary required" — so the disabled state points at the exact inputs to fix the instant the override value is typed.
...
- `.validation-error` (in [css](css)) — red border + tint + shake on the editable input itself. Applied to `verification-input` / `override-input` / `commentary-input` via `ng-class`. Triggers: `field.validationError` on all three; **`c.needsCommentary(field)` on both `override-input` and `commentary-input`** (live QA-without-commentary highlight); and `field.commentaryRequired` on commentary. So the Complete disabled state points at the exact input(s) to fix.
...
- **2026-07-17** — QA Override Complete-blocked UX fix (**UM-2534**): users typing a QA Override value saw Complete disabled with a generic "Fix validation errors" tooltip and **no visible indication that Commentary was the required fix** — the commentary red border was only set on blur (`field.commentaryRequired`), and the QA Override input / error sub-row never reflected the client-side rule. Added pure live getter **`c.needsCommentary(field)`** (single definition of the QA→Commentary rule; `hasValidationErrors` now delegates to it) and **`c.completeDisabledReason()`** for a specific tooltip. Wired `c.needsCommentary(field)` into `ng-class` on both the **QA Override** and **Commentary** inputs so they highlight red *immediately* (not on blur), switched the commentary placeholder to "Commentary required", and replaced the button's inline tooltip expression with `c.completeDisabledReason()`. Business rule unchanged — Commentary stays **required**; this is a pure UX/visibility fix ([clientscript.js](clientscript.js), [html](html)). **Pitfall:** keep `needsCommentary` pure (no mutation) — it runs in `ng-class`/`title` every digest.
- **2026-07-14** — Exclude Location line items: `fetchMapping` now skips any line item whose `section_name_final` **starts with** `Locations` ([serverscript.js](serverscript.js), in the `while (lineItemGr.next())` loop: `if (sectionName.indexOf('Locations') === 0) continue;`). Done as a **JS post-filter, not a GlideRecord query condition**, because ServiceNow's `NOT LIKE` is a "does not contain" match — it would over-exclude sections that merely contain `Locations` mid-string. Strict, case-sensitive prefix. **Pitfall:** if you move this to the DB layer, use a true starts-with-negation (not `NOT LIKE`), or it changes semantics.
- **2026-07-07** — Line-item query: removed the `setLimit(500)` cap and added `orderBy(sequence_final)` + `orderBy(field_name_final)` before `query()` ([serverscript.js](serverscript.js)). Root cause of "some fields missing per section": an extraction had 874 line items but the query was capped at 500 **with no orderBy**, so the DB truncated to an arbitrary 500 rows *before* the JS `lineItems.sort()` ran — dropping ~374 fields scattered unpredictably across sections. `parent` becoming a reference type was a red herring; the linkage was clean (all 874 on the correct parent, 0 unresolved). `CONFIG.limits.maxLineItems` removed; `CONFIG.limits` is now empty. **Pitfall:** if a cap is ever reintroduced, keep the `orderBy` so truncation is deterministic.
- **2026-06-27** — Document dropdown now updates on version change. Removed `track by doc.url` from the document `<select>` ng-options ([html](html)). Each version's documents carry different attachment URLs (`file_url = /sys_attachment.do?sys_id=<attachmentSysId>`), and reassigning both `c.documents` and `c.selectedDocument` in the same digest caused `ngOptions` with `track by` to leave a stale/blank selection that didn't match the loaded PDF. Tracking by object reference works because `c.selectedDocument` is always set to an actual member of `c.documents` (`documentList[0]` on load; matched member in cross-document nav). **Pitfall:** don't reintroduce `track by` here unless `c.selectedDocument` is guaranteed to be selected by that tracked key, not by reference.
