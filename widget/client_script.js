api.controller = function($scope, $location, $window, spUtil, $timeout) {
  ...
  // URL parameters
  var extractionSysId = $location.search().extractionSysID;
  var docSysID = $location.search().docSysID;
  ...
  function initializeWidget() {
    ...
    c.server.get({
      action: 'getDocuments',
      extractionSysId: extractionSysId,
      docSysID: docSysID
    }).then(function(response) {
      c.documents = response.data.documents || [];
      if (c.documents.length > 0 && !c.selectedDocument) {
        // Auto-select first document if available
        c.selectedDocument = c.documents[0].sys_id;
        c.loadDocument();
      }
      loadSourceMapping();
    }).catch(function(error) {
      c.isLoading = false;
      spUtil.addErrorMessage('Failed to load documents');
    });
  }
  ...
  c.loadDocument = function() {
    ...
    c.server.get({
      ...
      c.isLoading = false;
      spUtil.addErrorMessage('Error loading document');
    });
  };
